...
# ===== IMPORT ALL MODULES =====
from config import IMG_PATH, OUT_DIR, KEY_PATH
...
def main():
    ...
    # Encrypt modified plaintext (generates fresh key independently)
    crypto_diff = RubikCubeCrypto(Image.fromarray(orig_diff))
    enc_diff = crypto_diff.encrypt()
    ...
    # Compare with the original encrypted image (which was already encrypted)
