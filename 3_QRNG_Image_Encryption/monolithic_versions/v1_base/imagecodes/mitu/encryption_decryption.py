...
class RubikCubeCrypto:
    ...
    def encrypt(self, use_existing_key=False, tag=""):
        save_stage(
            np.array(Image.fromarray(self.work).convert("L")),
            f"{tag}01_enc_start.png"
        )
        ...
        for r in range(self.rounds):
            ...
            save_stage(
                np.array(Image.fromarray(self.work).convert("L")),
                f"{tag}enc_round{r + 1}_rows.png"
            )

            self.roll_cols(True)
            save_stage(
                np.array(Image.fromarray(self.work).convert("L")),
                f"{tag}enc_round{r + 1}_cols.png"
            )

            self.xor_pixels()
            save_stage(
                np.array(Image.fromarray(self.work).convert("L")),
                f"{tag}enc_round{r + 1}_xor.png"
            )

        save_stage(
            np.array(Image.fromarray(self.work).convert("L")),
            f"{tag}02_encrypted_final.png"
        )
    ...
    def decrypt(self, tag=""):
        ...
        for r in range(self.rounds):
            self.xor_pixels()
            save_stage(np.array(Image.fromarray(self.work).convert("L")),
                       f"{tag}dec_round{r+1}_xor.png")

            self.roll_cols(False)
            save_stage(np.array(Image.fromarray(self.work).convert("L")),
                       f"{tag}dec_round{r+1}_cols.png")

            self.roll_rows(False)
            save_stage(np.array(Image.fromarray(self.work).convert("L")),
                       f"{tag}dec_round{r+1}_rows.png")

        self.inverse_block_permute()

        save_stage(np.array(Image.fromarray(self.work).convert("L")),
                   f"{tag}04_decrypted_final.png")
...
# =========================================================
t0=time.time()
dec = RubikCubeCrypto(enc).decrypt(tag="base_")
t_dec=time.time()-t0
...
# =========================================================
# DIFFERENTIAL ATTACK (PLAINTEXT SENSITIVITY) — CORRECT
# =========================================================
...
enc_orig = crypto1.encrypt(tag="diff_orig_")
...
crypto2.load_key()
enc_diff = crypto2.encrypt(use_existing_key=True, tag="diff_mod_")
enc_diff_np = np.array(enc_diff.convert("L"))

save_stage(enc_orig_np, "diff_13_enc_original_for_npcr.png")
save_stage(enc_diff_np, "diff_14_enc_differential.png")
...
# =========================================================
# KEY SENSITIVITY ANALYSIS — CORRECT
# =========================================================

# Encrypt with original key
crypto_key1 = RubikCubeCrypto(orig)
enc_key1 = np.array(crypto_key1.encrypt(tag="key_orig_").convert("L"))
save_stage(enc_key1, "key_15_enc_key_original.png")
...
enc_key2 = np.array(crypto_key2.encrypt(use_existing_key=True, tag="key_pert_").convert("L"))
...
save_stage(enc_block, "block_05_enc_blocking_attack.png")

dec_block = np.array(
    RubikCubeCrypto(Image.fromarray(enc_block)).decrypt(tag="block_").convert("L")
)
save_stage(dec_block, "block_06_dec_blocking_attack.png")
...
save_stage(enc_sp, "sp_07_enc_salt_pepper.png")

dec_sp = np.array(
    RubikCubeCrypto(Image.fromarray(enc_sp)).decrypt(tag="sp_").convert("L")
)
save_stage(dec_sp, "sp_08_dec_salt_pepper.png")

# --- Gaussian ---
enc_gauss = gaussian_attack(enc_np, var=0.001)
save_stage(enc_gauss, "gauss_09_enc_gaussian.png")

dec_gauss = np.array(
    RubikCubeCrypto(Image.fromarray(enc_gauss)).decrypt(tag="gauss_").convert("L")
)
save_stage(dec_gauss, "gauss_10_dec_gaussian.png")

# --- QRNG-based noise ---
enc_qrng_noise = qrng_noise_attack(enc_np, qrng_bytes, strength=0.1)
save_stage(enc_qrng_noise, "qrng_noise_11_enc_qrng_noise.png")

dec_qrng_noise = np.array(
    RubikCubeCrypto(Image.fromarray(enc_qrng_noise)).decrypt(tag="qrng_noise_").convert("L")
)
save_stage(dec_qrng_noise, "qrng_noise_12_dec_qrng_noise.png")
