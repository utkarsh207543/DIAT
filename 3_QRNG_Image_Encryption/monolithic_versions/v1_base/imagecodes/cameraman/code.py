...
# =========================================================
# DIFFERENTIAL ATTACK (PLAINTEXT SENSITIVITY)
# =========================================================
...
enc_orig = np.array(
    RubikCubeCrypto(Image.fromarray(orig_np)).encrypt().convert("L")
)

# Encrypt slightly modified image
enc_diff = np.array(
    RubikCubeCrypto(Image.fromarray(orig_diff)).encrypt().convert("L")
)

# Save cipher images
save_stage(enc_orig, "13_enc_original_for_npcr.png")
save_stage(enc_diff, "14_enc_differential.png")

NPCR_val = NPCR(enc_orig, enc_diff)
UACI_val = UACI(enc_orig, enc_diff)

# =========================================================
# KEY SENSITIVITY ANALYSIS
# =========================================================

# --- Encrypt with original QRNG key ---
enc_key1 = np.array(
    RubikCubeCrypto(orig).encrypt().convert("L")
)
save_stage(enc_key1, "15_enc_key_original.png")

# --- Modify key slightly (1-bit flip) ---
qrng_bytes_perturbed = qrng_bytes.copy()
qrng_bytes_perturbed[0] ^= 1   # flip 1 bit in key

# Temporarily replace global qrng_bytes
qrng_bytes_backup = qrng_bytes
qrng_bytes = qrng_bytes_perturbed

# Encrypt again with modified key
enc_key2 = np.array(
    RubikCubeCrypto(orig).encrypt().convert("L")
)
save_stage(enc_key2, "16_enc_key_perturbed.png")

# Restore original QRNG key
qrng_bytes = qrng_bytes_backup

# --- Compute key sensitivity metrics ---
NPCR_key = NPCR(enc_key1, enc_key2)
UACI_key = UACI(enc_key1, enc_key2)

# Optional: SSIM between cipher images
...
save_stage(enc_block, "05_enc_blocking_attack.png")

dec_block = np.array(
    RubikCubeCrypto(Image.fromarray(enc_block)).decrypt().convert("L")
)
save_stage(dec_block, "06_dec_blocking_attack.png")
