...
def UACI(img1, img2):
    ...
    Standard UACI formula: (1/(M*N)) * sum(|I1(i,j) - I2(i,j)|/255) * 100
    Expected value: ~33.46% for good encryption
    """
    assert img1.shape == img2.shape
    M, N = img1.shape
    diff_sum = np.sum(np.abs(img1.astype(np.float64) - img2.astype(np.float64)) / 255.0)
    return (diff_sum / (M * N)) * 100
