...
class RubikCubeCrypto:
    ...
    def diffusion_layer(self, enc=True):
        """
        Additional diffusion layer for enhanced UACI
        Uses pixel chaining where each pixel is XORed with previous pixels
        This creates strong avalanche effect while remaining reversible
        
        Args:
            enc: True for encryption, False for decryption
        """
        if enc:
            # Forward diffusion - chain pixels left to right, top to bottom
            for i in range(self.m):
                for j in range(self.n):
                    # Get reference values (already processed pixels)
                    left = self.work[i, j-1] if j > 0 else self.Kr[i]
                    top = self.work[i-1, j] if i > 0 else self.Kc[j]
                    
                    # XOR current pixel with left and top neighbors
                    self.work[i, j] = (self.work[i, j] ^ left ^ top) & 0xFF
        else:
            # Reverse diffusion - must process in FORWARD order
            # Save original encrypted values before modifying
            temp = self.work.copy()
            for i in range(self.m):
                for j in range(self.n):
                    # Use the ORIGINAL encrypted neighbor values (from temp)
                    left = temp[i, j-1] if j > 0 else self.Kr[i]
                    top = temp[i-1, j] if i > 0 else self.Kc[j]
                    
                    # XOR to reverse the diffusion
                    self.work[i, j] = (temp[i, j] ^ left ^ top) & 0xFF
    ...
    def encrypt(self, use_existing_key=False, skip_block_permute=False):
        ...
        for r in range(self.rounds):
            ...
            self.roll_cols(True)
            self.xor_pixels()
            self.diffusion_layer(True)  # Additional diffusion for enhanced UACI
    ...
    def decrypt(self):
        ...
        for r in range(self.rounds):
            self.diffusion_layer(False)  # Reverse diffusion first
            self.xor_pixels()
