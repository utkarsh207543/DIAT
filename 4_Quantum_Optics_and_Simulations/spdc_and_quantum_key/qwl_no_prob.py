...
V_well = -1 * e  # Well depth in Joules (eV converted to J)
m_eff = 0.067 * m_e  # Effective mass of electron (e.g., GaAs)
num_levels = 3  # Number of energy levels to compute


# Define potential well
def potential(z, L):
    return 0 if -L / 2 <= z <= L / 2 else 1e10 * e  # Very high outside the well
...
def solve_schrodinger(L, V, m, levels):
    ...
    dz = z[1] - z[0]
    V_z = np.array([potential(zi, L) for zi in z])
...
plt.title("Quantum Well Wavefunctions")
