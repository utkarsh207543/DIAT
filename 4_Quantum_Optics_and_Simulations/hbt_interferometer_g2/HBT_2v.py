...
def classify_g2(g2_0, label=""):
    print(f"\n📌 {label} → g²(0) = {g2_0:.3f}")
    if g2_0 < 0.95:
        print("🟣 Antibunching: Sub-Poissonian (single-photon source)")
    elif g2_0 > 1.05:
...
# Main HBT Pipeline
# -----------------------------
def full_g2_pipeline(filename, bin_width=50e-9, max_delay=1e-6, zoom_ns=200):
    print("📥 Loading data...")
...
# Run the Pipeline
# -----------------------------
full_g2_pipeline("test5_bs.csv", bin_width=50e-9, max_delay=1e-6, zoom_ns=500)
