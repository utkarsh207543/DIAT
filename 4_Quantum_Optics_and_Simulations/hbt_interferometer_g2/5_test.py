...
# Parameters
# ----------------------------
filename = "test3.csv"  # Replace with your actual file
...
# ----------------------------
# g²(τ) for delta_t (Δt = t2 - t1)
# ----------------------------
def compute_g2_of_deltas(delta_ts, bin_width, max_delay):
    delays = []
    n = len(delta_ts)
    for i in tqdm(range(n), desc="Computing g²(Δt)"):
        for j in range(i+1, n):
            dt = delta_ts[j] - delta_ts[i]
            if dt > max_delay:
                break
            delays.append(dt)
            delays.append(-dt)  # for symmetry
...
while i < len(ch1_timestamps) and j < len(ch2_timestamps):
    ...
    if abs(dt) <= max_delay:
        ...
        i += 1
delta_ts = np.array(sorted(delta_ts))
bin_centers_delta, g2_delta = compute_g2_of_deltas(delta_ts, bin_width, max_delay)
