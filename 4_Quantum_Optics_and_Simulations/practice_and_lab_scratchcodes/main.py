...
# Plot three possible states
for i in range(3):
    state = np.abs(paths[i, :])**2
    plt.subplot(3, 2, 2 * i + 1)
    plt.plot(x_initial, state, label=f'State {i + 1}')
    plt.title(f'Probability Density for State {i + 1}')
    plt.xlabel('Position (x)')
    plt.ylabel('|ψ(x)|²')
    plt.legend()

    plt.subplot(3, 2, 2 * i + 2)
    plt.plot(np.real(paths[i, :]), np.imag(paths[i, :]), label=f'State {i + 1}')
    plt.title(f'Trajectory in Complex Plane for State {i + 1}')
    plt.xlabel('Re(x)')
    plt.ylabel('Im(x)')
    plt.legend()
