#!/usr/bin/env python3
"""
Run all QRNG post-processing techniques - Updated
Execute this script to process your QRNG data through the updated pipeline workflow.
"""

from main_processor import QRNGPostProcessingSuite
import sys
import os

def main():
    # Check if input file is provided
    if len(sys.argv) > 1:
        input_file = sys.argv[1]
    else:
        input_file = "Utkarsh_QRNG_10.txt"
        
    # Check if file exists
    if not os.path.exists(input_file):
        print(f"❌ Error: Input file '{input_file}' not found!")
        print("Please ensure your QRNG data file is in the current directory.")
        return
        
    print("🚀 Starting QRNG Post-Processing Pipeline")
    print(f"📁 Input file: {input_file}")
    print("🔄 Following updated workflow:")
    print("1. Read raw data → Save Raw_QRNG.txt")
    print("2. Apply Von Neumann debiasing → Save base")
    print("3. Apply all techniques to debiased base")
    print("4. Entropy amplification to 500K bits")
    print("5. Save with postprocessing names")
    
    try:
        # Create and run processor with updated method
        processor = QRNGPostProcessingSuite(input_file)
        processor.process_qrng_pipeline()  # Updated method name
        
        print("\n🎉 Processing completed successfully!")
        print("\n📋 Next steps for NIST testing:")
        print("1. Navigate to the 'qrng_outputs' directory")
        print("2. Use the .bin files with the NIST Statistical Test Suite")
        print("3. Compare results across different techniques")
        print("4. Review the Processing_Pipeline_Report.txt for detailed analysis")
        
    except Exception as e:
        print(f"❌ Error during processing: {e}")
        import traceback
        traceback.print_exc()
        return 1
        
    return 0

if __name__ == "__main__":
    exit(main())
